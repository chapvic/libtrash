...
/* This wrapper is less "faithfully" done the other ones. Rather than invoking
 * the real GNU libc openat(), it simply invokes our "doctored" open() which,
 * in turn, might call the actual GNU libc open(). See important note in the 
 * CHANGE.LOG about our wrapping of the *at() functions. */
